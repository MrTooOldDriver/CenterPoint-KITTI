...
class PubTracker(object):
  def __init__(self,  hungarian=False, max_age=0):
    ...
    self.VOD_CLS_VELOCITY_ERROR = VOD_CLS_VELOCITY_ERROR
  ...
  def step_centertrack(self, results: List, time_lag: float):
    if len(results) == 0:
      self.tracks = []
      return []
    else:
      temp = []
      for det_obj in results:
        # filter out classes not evaluated for tracking 
        if det_obj.cls_type.lower() not in VOD_TRACKING_NAMES:
          continue
        det = {}
        det['ct'] = np.array([det_obj.loc[2], -det_obj.loc[0]])
        # -1?
        det['tracking'] = np.array([det_obj.v_x, det_obj.v_y]) * -1 * time_lag
        det['label_preds'] = det_obj.cls_id
        det['detection_name'] = det_obj.cls_type.lower()
        det['origin_obj'] = det_obj
        temp.append(det)
    ...
    max_diff = np.array([self.VOD_CLS_VELOCITY_ERROR[box['detection_name']] for box in results], np.float32)
