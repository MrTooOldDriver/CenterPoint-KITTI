...
class IASSD_Head_DLP(PointHeadTemplate):
    """
    ...
    def __init__(self, num_class, input_channels, model_cfg, predict_boxes_when_training=False, **kwargs):
        ...
        # for ponder loss calculation
        self.current_halting_layer_number = None
...
# 
    ...
    def assign_targets(self, input_dict, j):
        """
        Args:
            input_dict:
                batch_size: int
        ...
        center_targets_dict = self.assign_stack_targets_IASSD(
            points=input_dict['%s_centers'% j].detach(), 
        ...
        if extra_method is not None and extra_method.NAME == 'extend_gt':
            ...
            if extra_method.get('ASSIGN_TYPE', 'centers') == 'centers_origin': 
                points = input_dict['%s_centers_origin'% j].detach()
            else:
                points = input_dict['%s_centers'% j].detach() #default setting
    ...
    def get_loss(self, tb_dict=None):
        ...
        for j in [0,1,2]:
            self.current_halting_layer_number = j
            tb_dict = {}
            ...
            if self.model_cfg.LOSS_CONFIG.get('LOSS_INS', None) is not None:
                ...
                assert ('%s_sa_ins_preds' % self.current_halting_layer_number in self.forward_ret_dict) and ('sa_ins_labels' in self.forward_ret_dict)
            ...
            tb_dict['det_loss'] = point_loss.item()   
            tb_dict_list[j] = tb_dict
            point_loss_list[j] = point_loss
        return point_loss_list, tb_dict_list
    ...
    def get_vote_loss_loss(self, tb_dict=None):
        ...
        centers_origin = self.forward_ret_dict['%s_centers_origin'% self.current_halting_layer_number]
        ctr_offsets = self.forward_ret_dict['%s_ctr_offsets'% self.current_halting_layer_number]
        centers_pred = centers_origin + ctr_offsets
    ...
    def get_center_cls_layer_loss(self, tb_dict=None):
        ...
        point_cls_preds = self.forward_ret_dict['%s_center_cls_preds' % self.current_halting_layer_number].view(-1, self.num_class)
    ...
    def get_sa_ins_layer_loss(self, tb_dict=None):
        ...
        sa_ins_preds = self.forward_ret_dict['%s_sa_ins_preds' % self.current_halting_layer_number]
    ...
    def generate_center_ness_mask(self):
        ...
        centers = self.forward_ret_dict['%s_centers' % self.current_halting_layer_number][:,1:]
    ...
    def get_center_box_binori_layer_loss(self, tb_dict=None):
        ...
        point_box_preds = self.forward_ret_dict['%s_center_box_preds' % self.current_halting_layer_number]
    ...
    def get_corner_layer_loss(self, tb_dict=None):
        pos_mask = self.forward_ret_dict['center_cls_labels'] > 0
        gt_boxes = self.forward_ret_dict['center_gt_box_of_fg_points']
        pred_boxes = self.forward_ret_dict['%s_point_box_preds' % self.current_halting_layer_number]
    ...
    def forward(self, batch_dict):
        ...
        if self.training: 
            ...
            post_sa_list = [p for p in range(batch_dict['halting_step'].cpu().numpy().max())]
        for j in post_sa_list:

            center_features = batch_dict['%s_centers_features'% j]
            center_coords = batch_dict['%s_centers'% j]
            ...
            ret_dict.update({'%s_center_cls_preds'% j: center_cls_preds,
                        '%s_center_box_preds'% j: center_box_preds,
                        '%s_ctr_offsets'% j: batch_dict['%s_ctr_offsets' % j],
                        '%s_centers'% j: batch_dict['%s_centers'% j],
                        '%s_centers_origin'% j: batch_dict['%s_centers_origin'% j],
                        '%s_sa_ins_preds'% j: batch_dict['%s_sa_ins_preds'% j],
                        '%s_box_iou3d_preds'% j: box_iou3d_preds,
                        })
            if self.training or self.debug:
                targets_dict = self.assign_targets(batch_dict, j)
            ...
            if not self.training or self.predict_boxes_when_training or \
                ...
                batch_dict['%s_batch_cls_preds'% j] = point_cls_preds
                batch_dict['%s_batch_box_preds'% j] = point_box_preds
                batch_dict['%s_box_iou3d_preds'% j] = box_iou3d_preds
                batch_dict['%s_batch_index'% j] = center_coords[:,0]
                batch_dict['%s_cls_preds_normalized'% j] = False

                ret_dict['%s_point_box_preds'% j] = point_box_preds
