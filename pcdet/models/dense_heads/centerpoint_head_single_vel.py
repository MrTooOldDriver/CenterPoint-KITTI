...
class CenterHeadVel(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        vel_preds = self.conv_vel(spatial_features_2d)

        cls_preds = cls_preds.permute(0, 2, 3, 1).contiguous()  # [N, H, W, C]
        box_preds = box_preds.permute(0, 2, 3, 1).contiguous()  # [N, H, W, C]
        vel_preds = vel_preds.permute(0, 2, 3, 1).contiguous()  # [N, H, W, C]

        self.forward_ret_dict['cls_preds'] = cls_preds
        self.forward_ret_dict['box_preds'] = box_preds
        self.forward_ret_dict['vel_preds'] = vel_preds
        
        if self.training:
            targets_dict = self.assign_targets(
                gt_boxes=data_dict['gt_boxes'],
                gt_boxes_vel=data_dict['gt_boxes_vel'],
            )
            self.forward_ret_dict.update(targets_dict)

        if not self.training or self.predict_boxes_when_training:
            batch_cls_preds, batch_box_preds, batch_vel_preds = self.generate_predicted_boxes(
                batch_size=data_dict['batch_size'],
                cls_preds=cls_preds, box_preds=box_preds, vel_preds=vel_preds, dir_cls_preds=None
            )
            data_dict['batch_cls_preds'] = batch_cls_preds
            data_dict['batch_box_preds'] = batch_box_preds
            data_dict['batch_vel_preds'] = batch_vel_preds
    ...
    def assign_targets(self, gt_boxes, gt_boxes_vel):
        """Generate targets.

        Args:
            gt_boxes: (B, M, 8) box + cls 
            gt_boxes_vel (B, M, 2) v_x, v_y
        Returns:
            Returns:
        ...
        gt_boxes_vel_3d = gt_boxes_vel

        heatmaps, anno_boxes, inds, masks, anno_boxes_vel = multi_apply(
            self.get_targets_single, gt_bboxes_3d, gt_labels_3d, gt_boxes_vel_3d)
        ...
        # transpose anno_boxes
        # anno_boxes = np.array(anno_boxes).transpose(1, 0).tolist()
        anno_boxes_vel = [[anno_vel[0] for anno_vel in anno_boxes_vel]]
        anno_boxes_vel = [torch.stack(anno_boxes__vel_) for anno_boxes__vel_ in anno_boxes_vel]
        ...
        all_targets_dict = {
            ...
            'inds': inds,
            'masks': masks,
            'anno_boxes_vel': anno_boxes_vel
        }
        
        return all_targets_dict

    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d, gt_boxes_vel_3d):
        ...
        task_vel = [gt_boxes_vel_3d]

        draw_gaussian = draw_heatmap_gaussian
        heatmaps, anno_boxes, inds, masks = [], [], [], []
        anno_boxes_vel = []

        assert gt_labels_3d.shape[0] == gt_boxes_vel_3d.shape[0]
        ...
        for idx in range(1):
            ...
            anno_box = gt_bboxes_3d.new_zeros((max_objs, 8),
                                              dtype=torch.float32)
            anno_box_vel = gt_boxes_vel_3d.new_zeros((max_objs, 2),
                                              dtype=torch.float32)
            ...
            for k in range(num_objs):
                ...
                if width > 0 and length > 0:
                    ...
                    v_x = task_vel[idx][k][0]
                    v_y = task_vel[idx][k][1]
                    ...
                    anno_box_vel[new_idx] = torch.cat([
                        torch.tensor([v_x, v_y], device=device),
                    ])
            ...
            masks.append(mask)
            inds.append(ind)
            anno_boxes_vel.append(anno_box_vel)
        return heatmaps, anno_boxes, inds, masks, anno_boxes_vel

    def generate_predicted_boxes(self, batch_size, cls_preds, box_preds, vel_preds, dir_cls_preds=None):
        """
        Args:
            batch_size:
            cls_preds: (N, H, W, C1)
        ...
        batch_vel_preds = vel_preds.view(batch, H*W, -1)
        return batch_cls_preds, batch_box_preds, batch_vel_preds

    def get_loss(self):
        cls_loss, tb_dict = self.get_cls_layer_loss()
        box_loss, tb_dict_box = self.get_box_reg_layer_loss()
        tb_dict.update(tb_dict_box)
        vel_loss, tb_dict_vel = self.get_vel_reg_layer_loss()
        tb_dict.update(tb_dict_vel)
        rpn_loss = cls_loss + box_loss + vel_loss

        tb_dict['rpn_loss'] = rpn_loss.item()
        return rpn_loss, tb_dict
    
    def get_vel_reg_layer_loss(self):
        # Regression loss for v_x and v_y
        target_vel, inds, masks = self.forward_ret_dict['anno_boxes_vel'][0], self.forward_ret_dict['inds'][0], self.forward_ret_dict['masks'][0]

        ind = inds
        num = masks.float().sum()
        pred = self.forward_ret_dict['vel_preds'] # N x (HxW) x 2
        pred = pred.view(pred.size(0), -1, pred.size(3))
        pred = self._gather_feat(pred, ind)
        mask = masks.unsqueeze(2).expand_as(target_vel).float()
        isnotnan = (~torch.isnan(target_vel)).float()
        mask *= isnotnan

        code_weights = self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS['vel_code_weight']
        bbox_weights = mask * mask.new_tensor(code_weights)

        loc_loss = l1_loss(
            pred, target_vel, bbox_weights, avg_factor=(num + 1e-4))

        loc_loss = loc_loss * self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS['vel_weight']
        box_loss = loc_loss
        tb_dict = {
            'vel_loss_loc': loc_loss.item()
        }
